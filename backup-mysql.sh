...
days_of_backups=7 # stores during one week
backup_owner="backup"
parent_dir="/backups/mysql"
defaults_file="/etc/mysql/backup.cnf"
todays_dir="${parent_dir}/$(date +%Y%m%d)"
log_file="${todays_dir}/backup-progress.log"
encryption_key_file="${parent_dir}/scripts/encryption_key"
now="$(date +%Y%m%d_%H%M)"
...
take_backup () {
    ...
    if grep -q -s "to_lsn" "${todays_dir}/xtrabackup_checkpoints"; then
        echo "#########################################################" >> "${log_file}"
        echo " Incremental Backup is starting at ${now} .... " >> "${log_file}"
        echo "#########################################################" >> "${log_file}"
    else
        echo "#########################################################" >> "${log_file}"
        echo " Full Backup is starting at ${now} .... " >> "${log_file}"
        echo "#########################################################" >> "${log_file}"
    fi
