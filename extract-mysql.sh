...
backup_owner="backup"
encryption_key_file="/backups/mysql/scripts/encryption_key"
...
sanity_check () {
    ...
    # Check file backup is exist        
    count=`ls -1 *.xbstream 2>/dev/null | wc -l`
    if [ $count -lt 1 ]; then
        error "There is no file *.xbstream exist, please doule-check and try again !!!"
    fi
